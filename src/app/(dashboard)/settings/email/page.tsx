            ...
            <form onSubmit={handleSave} className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
              ...
              {config.provider === "IMAP" && (
                <>
                  ...
                  {[
                    { key: "imapHost" as const, label: "IMAP Sunucu", placeholder: "imap.gmail.com" },
                    { key: "imapUser" as const, label: "E-posta Adresi", placeholder: "it-destek@firma.com" },
                    { key: "imapPassword" as const, label: "App Password", placeholder: "xxxx xxxx xxxx xxxx", type: "password" },
                  ].map((field) => (
                    <div key={field.key}>
              ...
              {config.provider === "MICROSOFT_GRAPH" && (
                <>
                  ...
                  {[
                    { key: "tenantId" as const, label: "Tenant ID", placeholder: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" },
                    { key: "clientId" as const, label: "Client ID (Application ID)", placeholder: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" },
                    { key: "clientSecret" as const, label: "Client Secret", placeholder: "Gizli anahtarı girin", type: "password" },
                    { key: "mailbox" as const, label: "Posta Kutusu (UPN)", placeholder: "it-support@firma.com" },
                  ].map((field) => (
                    <div key={field.key}>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{field.label}</label>
                      <input
